...
function get_knowledge_asset_features_values(triples) {
  let feature_values = [];
  let vals = [];
  let features = triples.filter(t => t.object == "KnowledgeAssetFeature" && t.predicate == "subClassOf").map(t=>t.subject);
  for(let f of features){
    vals = triples.filter(t => t.object == f && t.predicate == 'subClassOf' && t.subject.includes(f+'Value')).map(t => t.subject.replace(f+'Value',''));
    vals = vals.map(v => ({ type: 'KnowledgeAssetFeature', name: v}))
    feature_values.push(...vals)
  }
  return feature_values;
}


function get_feature_value(triples,feature,feature_assignment){
  let value = triples.find(t => t.subject == feature_assignment && t.predicate == "type" && t.object.includes(feature+'Value')).object;
  value = value.replace(feature+'Value','');
  return value;
}
...
function get_knowledge_asset_features_relationships(triples) {
  ...
  for (let feature of features) {
    feature_relationships.push(...triples.filter(t => t.predicate == "has" + feature).map(t => ({ source: t.subject, target: get_feature_value(triples,feature,t.object), type: t.predicate })));
  }
...
function getNodes(triples) {
  ...
  return [].concat.apply([], [knowledgeAssetNodes, personNodes, get_knowledge_asset_features_values(triples)]);
}
